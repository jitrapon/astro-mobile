...
This skill assumes **Claude Opus at `high` effort** (Claude Code's default — `high` is equivalent to omitting the parameter). One model drives every mode; there is no per-item model assignment. `high` covers planning, implementation, and complex reasoning across all three modes — confirm you're on Opus before starting and don't downgrade the model mid-branch.
