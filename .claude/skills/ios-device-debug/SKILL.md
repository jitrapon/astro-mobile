...
1. **Xcode 27 is running with the project open.** The bridge enumerates tools
   ...
   `open -a "/Applications/Xcode-27.0.0.app" iosApp/iosApp.xcodeproj`
...
3. **The `xcode` MCP server (mcpbridge) is registered and connected.** It is a
   ...
   ```bash
   DEV=/Applications/Xcode-27.0.0.app/Contents/Developer   # adjust to your Xcode
...
```bash
export DEVELOPER_DIR=/Applications/Xcode-27.0.0.app/Contents/Developer  # adjust
UUID=<sim-uuid>
...
- **`Connected · tools fetch failed` / `tools/list` timeout** → on **27.0 Beta 5
  ...
  **Fix** — `sudo` needs a real TTY, so these run in Terminal.app, not through the
  ...
  ```bash
  DEV=/Applications/Xcode-27.0.0.app/Contents/Developer   # adjust
