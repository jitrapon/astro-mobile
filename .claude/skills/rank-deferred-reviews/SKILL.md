...
3. **Extract per-issue facts.** For each issue, parse the body for:
   ...
   - **Plan task** — which plan task, if any, the issue belongs to, and that task's full scope.
     ...
     relevance from the index first. Fetch astro-docs once per run with a shallow clone over SSH,
     falling back to the `gh api` download (SSH first; `BatchMode=yes` makes a key that is not
     loaded in `ssh-agent` fail in seconds instead of hanging on a passphrase prompt nobody can
     answer, and the `gh api` download covers that case), then search per issue with `<N>` its
     number:
     ...
     carries its whole goal inline. Record `none` when nothing maps. If both fetches fail, score
