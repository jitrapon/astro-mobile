...
- [x] 6. Add `data/calendar/CalendarScreenQuery.kt` with `observeScreen(request)`: find-or-create the per-request `MutableStateFlow` under one exclusive `Mutex` and hand it back **before** any network work; ask `ScreenCachePolicy` what to do with the cached entry; set `isFetching = true` on whatever is already showing rather than collapsing to a skeleton; run the exchange through `SingleFlightRunner`; write through to `ScreenCache` **on dispatch**, never inside `CalendarScreenApi`; publish `Failed` with the last loaded response still attached. Cancellation emits nothing and never produces `Failed`.
- [x] 7. Add `refetchScreen(request)` (exchanges even inside the staleness window) and `invalidateScreens(matches)` to `CalendarScreenQuery`. One map, one `Mutex`, no reader/shared-lock optimisation. Two invariants the ADR's own test list does not pin down, both of which this item must settle:
...
- [x] 8. Extend `CalendarScreenRepository` with `observeCalendarScreen` / `refetchCalendarScreen` / `invalidateCalendarScreens`, each a thin forward to `CalendarScreenQuery`; leave `fetchCalendarScreen` untouched. Replace the KDoc paragraph asserting the repository holds no state with one stating what the cache now guarantees — chiefly that a response is only ever published under the request that asked for it — and update `CalendarScreenRepositoryTest`'s class KDoc, which currently pins the opposite. The three new members are `internal`, not public: the generated header emits a non-exported library type under a mangled name rather than dropping it (`SharedKotlinx_serialization_coreKSerializer` is already there), so a public `Flow`-returning member would put a coroutines type on the framework surface — the one thing item 13's guard forbids. Swift reaches the seam through item 11's adapter and Android through item 10's view model, both inside this module.
- [x] 9. Wire the graph in `di/DataLayerModule.kt`: a `CoroutineDispatcher` binding, a `CoroutineScope(SupervisorJob() + dispatcher)` with `onClose { it?.cancel() }`, the ticker, cache, policy, single-flight runner and query; the repository's constructor takes the query. `DataLayerModule.kt` is the **only** file permitted to name a dispatcher. Add the matching `DependencyGraphResolutionTest` assertion for every new binding **in the same commit** — the graph has no compile-time validation, so an unasserted binding has no signal at all.
- [x] 10. Add `CalendarViewModel` in the shared module, consuming `observeCalendarScreen` and exposing its own UI state. It owns no `CoroutineScope` it did not receive, and exposes no library type. Lands in `presentation/calendar/` — a layer beside `data/`, not inside it — as the view model plus its own `CalendarUiState` (content / `isLoading` / failure), which is a projection rather than a re-export: it flattens the three query cases so no renderer downcasts to ask "is there content" or "is it busy", and it drops `servedFromCache`, because a remembered screen and an identical fetched one must paint identically. The class is **public and `@HiddenFromObjC`**, not `internal` as item 8's reasoning would otherwise imply: `:androidApp` is a separate module that `internal` cannot reach, while its `StateFlow` state and `CoroutineScope` constructor would each put a coroutines type on the framework surface item 13 guards. The state is `stateIn(scope, WhileSubscribed())` rather than a `MutableStateFlow` filled from `init` — nothing is observed until something collects, and starting network work in a constructor is the defect the armed `structured-coroutines` rules exist to catch.
- [x] 11. Settle D-24 and add the `iosMain` adapter that lets Swift collect the state flow, plus the `DependencyGraph` facade method that hands it out. A Kotlin `Flow` does not bridge to Swift — only `suspend` functions do — so the adapter converts subscription into something Swift can drive and returns a cancellation handle. Hand-rolled; **do not add SKIE**. Record the decision and its rejected alternative in the adapter's KDoc. **What Swift receives is `CalendarUiState`, not `CalendarScreenQueryState`:** each subscription builds a `CalendarViewModel` on its own child scope and forwards that view model's state, so the query-to-UI projection stays stated once in `toCalendarUiState()` rather than being restated by every SwiftUI screen, and Swift reads three plain fields with no downcast. The view model stays `@HiddenFromObjC`; the adapter (`CalendarScreenObserver.observe(request, onState)` → `CalendarScreenSubscription.cancel()`) takes and returns only shared-module types. Delivery is on the main thread via a scope bound in an iOS `di/*Module.kt` — the only file kind permitted to name a dispatcher — and is never synchronous inside `observe`, so a Swift caller always holds the handle before the first state reaches it. That scope is cancelled on graph teardown.
- [x] 12. Point `iosApp/iosApp/ContentView.swift`'s existing placeholder at the item-11 adapter instead of the one-shot fetch, preserving its discipline: cancellation settles nothing, and the delivered `CalendarUiState` is read through its `content` / `isLoading` / `failure` fields with no downcast of the observation's cases. **Scope guard:** this is boundary verification, not UI work — §1 excludes platform UI and #136 owns the real shell. It exists only because the `xcodebuild` app build is the one gate that type-checks Kotlin-declared symbols at their Swift call sites, so an adapter with no Swift caller is unverified at exactly the boundary D-24 exists to settle. Keep the edit to the smallest call site that compiles; behavioural coverage is §5.11's, not this item's.
- [x] 13. Add the mechanical guard that the linked framework header exposes none of this layer's implementation types. **This check does not exist yet** — the invariant is asserted only in prose today — so it must be built, not wired up: link the framework and assert the generated `shared.h` names no Koin, Ktor, `Flow`, or `internal` query-layer symbol. Landed as `:shared:verifyFrameworkHeaderSurface`, wired into `check` and classified into `verifyIos`. Two design points differ from this item's first wording. Library types are matched by the mangled klib prefix Kotlin/Native gives them (`SharedKoin_`, `SharedKtor_`, `SharedKotlinx_coroutines_`), not by the bare word, because the public `initKoin` entry point contains `Koin` and carries no Koin type. Query-layer names are derived from **location** (`data/query/` plus `CalendarScreenQuery.kt`), not from `internal` visibility: the compiler already keeps `internal` off the header, so a visibility-derived list could only fire on a compiler bug, and it would stop covering a type the moment its `internal` were deleted.
- [x] 14. Update `.claude/CLAUDE.md`: the "Key patterns" entry describing `CalendarScreenRepository` as "a **stateless forwarding boundary** today — no cached screen, no refresh policy" becomes false with item 8, and the new `data/query/` package needs naming in the package-layout entry, alongside the `presentation/calendar/` package item 10 adds. Item 13 adds two more: the Koin entry's claim that Koin and Ktor stay off the header should name `verifyFrameworkHeaderSurface` as its enforcement, and the CI bullet for `verify-ios` should list it in `verifyIos` (its xcodebuild step now reuses a framework that guard already linked, rather than adding the link).
...
- [x] 8. Repository: the three new methods forward to the query and nothing more; `fetchCalendarScreen`'s existing cases still pass unchanged; the existing case asserting two different requests get their own answers still holds.
- [x] 9. `DependencyGraphResolutionTest` resolves every new binding and asserts each is a singleton; the layer's `CoroutineScope` is cancelled when the graph is torn down, asserted the way the existing engine-release case asserts `isActive`.
- [x] 10. `CalendarViewModel`: it maps each `CalendarScreenQueryState` onto its UI state, stops collecting when its scope ends, and delivers no state after cancellation. The mapping cases are exhaustive and need no coroutines; the two time-dependent ones run the real stack. The cancellation case is kept from passing vacuously by waiting — through a second observer of the same request — for the abandoned exchange to actually land before asserting that nothing reached the ended scope, since "no screen was painted" is otherwise equally true of a case that finished before the backend answered.
- [x] 11. The D-24 adapter, in `iosTest` so it runs on the simulator — and this is where the adapter's *behaviour* is pinned, because §5.12 only proves it compiles: a subscription delivers successive states to the callback — the load before the screen, then the screen; the returned handle stops delivery and does not leak the subscription's coroutine (the delivery scope has no live children afterwards, and a later publication for the same request reaches nothing); cancelling mid-flight delivers nothing further and reports no failure, since cancellation is not an error, proven by waiting through a second observer for the abandoned exchange to land; and cancelling twice is safe and does not disturb a sibling subscription. `DependencyGraphTest` resolves the observer as a singleton and asserts its delivery scope is cancelled on `stop()`.
- [x] 12. `./gradlew :shared:iosSimulatorArm64Test` green, and the CLAUDE.md `xcodebuild` app-target build compiles `ContentView.swift` against the regenerated framework header — the only gate that type-checks Kotlin-declared symbols at their Swift call sites. **This is a compile gate only**; it cannot catch a runtime subscription or cancellation regression, which is why §5.11 carries the behavioural assertions rather than leaving them to the app build.
- [x] 13. The new header guard fails when a forbidden symbol is present. Prove it by construction — temporarily exposing a Koin or `Flow` type must turn it red — so the check is known to be capable of failing rather than merely green. Proven three ways against the final task: a scratch public class returning `Flow` / `StateFlow` / `Koin` / `HttpClient` was flagged on each member line; dropping `@HiddenFromObjC` from `CalendarViewModel` was flagged as `SharedKotlinx_coroutines_coreCoroutineScope` / `StateFlow`; and dropping `internal` from `Ticker` — a type that mentions no library type, so only the location rule can see it — was flagged as `SharedTicker`. Each probe was reverted and the real header scans clean against all 12 derived query-layer types. The task also self-tests its matcher on a synthetic header on every run.
- [x] 14. `./gradlew check` green end to end: `ktfmtCheck`, `detekt` (including the now-armed `structured-coroutines` Flow rules), `verifyCheckPartition`, and both test targets. **No `@Suppress` and no detekt baseline** — a finding is cleared by refactoring. If a weight-0 `(UNVERIFIED)` Flow rule fires, judge it as a true or false positive and record which; promote it to the blocking tier only on a confirmed true positive with no false one. Judged so
  far, both **false positives, neither promoted**: `ExternalScopeLaunch` (SCOPE_003) fires on
  `CalendarScreenQuery.invalidateScreens` but not on `observeScreen`, which does the identical
  `scope.launch` from a suspend *lambda* — it keys on syntax, not on the escape it claims to catch,
  and both remedies it suggests are wrong here (`coroutineScope` would make an invalidation wait out
  every affected screen's round trip; the function cannot be non-suspend because it takes a `Mutex`
  and the cache suspends). It reports, and is left reporting rather than contorted around.
  `MutableFlowExposed` (FLOW_010) fired on a `MutableStateFlow` held by a *file-private* class, so
  there was no public surface to hide; the encapsulation it pointed at was adopted on its own merits
  (`ObservedScreen` now owns its mutations behind intent-named operations) and the finding is gone. Final run: `./gradlew check` green (partition guard 46/46), with detekt, the
  simulator tests and `verifyFrameworkHeaderSurface` force-rerun rather than served from cache — 88
  host and 96 simulator tests, 0 failures; header clean against all 12 query-layer types; the only
  weight-0 finding left is the already-judged `ExternalScopeLaunch` on `invalidateScreens`.
