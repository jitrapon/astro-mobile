...
- **The shrunk app is run via a Gradle Managed Device**, on a new host-portable CI job. The tested
  build type is `minifiedTest` — `initWith(release)`, so the same R8 files and release signing —
  rather than `release` itself: AGP builds the test APK without the classes the app's classpath
  carries, yet minifies the app counting only the app's own reach (issuetracker 126429384), so the
  tests die on classes and members R8 removed or reshaped. `minifiedTest` departs from `release` in
  two ways only: keep rules **generated** by R8's TraceReferences for exactly what the androidTest
  classes reference (names pinned, access widening allowed, kotlinx.serialization and `$$serializer`
  rules dropped so serializer stripping stays detectable), and `minSdk` 24, so no desugared `$-CC`
  companion — which no keep can name — goes missing. A run that executes zero tests fails, because
  UTP reports a runner that crashed before its first test as `tests="0"` and a green build.
...
- [x] A5a. Add a `minifiedTest` build type — `initWith(getByName("release"))`, `matchingFallbacks += "release"`, `testProguardFiles("proguard-test-rules.pro")` (the class-named `-dontwarn` for `MustBeClosed`; the release build type carries no test rule) — and point `testBuildType` and the managed device at it. Under `androidComponents`, set its `minSdk` to 24 and register `GenerateInstrumentedTestKeepRules`: R8's TraceReferences over the androidTest classes (source) against the app's pre-R8 classes (target), emitting `-keep,allowaccessmodification` rules with names pinned, dropping every kotlinx.serialization / `$$serializer` rule, appended to the variant's `proguardFiles`. Re-point the root CI partition from the release unit-test/lint forms to the `minifiedTest` ones.
- [x] A5b. Fail an instrumented run that executes zero tests or never finishes, as actions on every `AndroidTestTask` itself (no `-x` can separate them from the run): clear stale `TEST-*.xml` first, then fail when the summed `<testsuite tests>` count is zero — a runner that crashes in `onStart` is reported by UTP as `tests="0"` and a green build — naming the likely cause; and a 20-minute task `timeout`, since a runner that crashes in `onCreate` never signals completion and `am instrument -w` blocks forever.
- [x] A6. Refactor `AppShellTest` off `androidx.compose.ui:ui-test-manifest` — a `debugImplementation` artifact absent from the release variant — by replacing `createComposeRule()` with `createAndroidComposeRule<MainActivity>()`, which uses the app's own manifest-declared activity.
- [x] A7. Add `MinifiedAppSmokeTest` (`androidApp/src/androidTest`): launch `MainActivity` and wait for the shell to settle on its failure placeholder — the one settled state the minified build reaches deterministically, since only the debug overlay permits cleartext to the development backend — proving the graph started, the repository resolved and the shell composed under R8. Black-box: it references only `MainActivity` and inlined tag constants, so it adds no generated keep. Extended in C8 once the registry exists.
- [ ] A8. Add the `verify-android-release` job to `.github/workflows/ci.yml` (`ubuntu-latest`): enable KVM via a udev rule, generate an ephemeral keystore with `keytool` and export the four `ASTRO_KEYSTORE_*` variables, then run `:androidApp:aospAtd34MinifiedTestAndroidTest` (which A5b's zero-test gate finalizes). Comment it as artifact/runtime coverage deliberately outside `verifyCheckPartition`, the same classification `:androidApp:assemble` carries.
...
- [ ] C8. Extend `MinifiedAppSmokeTest` to decode the vendored contract fixture through the app's real `BackendJson` codec, resolve every decoded body and event component through the Android registry, and dispatch one action of each of the five types — the polymorphic-deserialization path R8 would break, exercised inside the shrunk APK.
...
- [x] A5a. `./gradlew :androidApp:aospAtd34MinifiedTestAndroidTest` (credentials exported) executes a non-zero number of tests — read from the JUnit XML, not inferred from BUILD SUCCESSFUL. `mapping/release/configuration.txt` carries neither the generated rules nor the test rules, and `release`'s `usage.txt` still lists `androidx.tracing.Trace` as removed (so nothing test-driven reaches what ships). The generated file holds no kotlinx.serialization / `$$serializer` rule and no wildcard, and its only `io.jitrapon` rules name types the tests reference. No package-wide `-dontwarn` in `proguard-test-rules.pro`. `./gradlew verifyCheckPartition check` stays green.
- [x] A5b. Reproduce both failure modes and watch the gate catch each: with the generated rules unwired, the runner dies in `onCreate` and the run hangs — a temporarily shortened timeout fails it with `Timeout has been exceeded` and tears the emulator down; with only `androidx.tracing.Trace` kept, it dies in `onStart` and reports `tests="0"` — the run now fails naming zero executed tests. Restore and confirm the real run passes with its tests counted.
...
- [ ] C8. The extended `MinifiedAppSmokeTest` passes on the managed device. Confirm it has teeth: with `isMinifyEnabled = true` and a keep rule the decode needs deliberately removed, the test fails — then restore.
