...
- [ ] 4a. `gradle/libs.versions.toml` exists and Gradle configuration resolves (`./gradlew help`) with the lint tasks reading their versions from it; `grep -n 'no Gradle version catalog' .claude/CLAUDE.md` (case-insensitive) is empty.
- [ ] 4b. `./gradlew verifyKtfmtAlignment` passes; temporarily skewing `ktfmt-cli` in the catalog makes it fail with the drift message (revert after).
- [ ] 4c. `./gradlew checkNoDetektBaseline` passes on the clean tree and fails when a dummy `detekt-baseline.xml` is added (removed after); `ForbiddenSuppress` is active in the effective Detekt config; `./gradlew check` depends on `checkNoDetektBaseline`.
- [ ] 4d. `./gradlew resolveLintTools` writes `.gradle/lint-tools.properties` with the four keys (`ktfmt.jar`/`ktfmt.version`/`detekt.jar`/`detekt.version`); `./gradlew installGitHooks` sets `git config core.hooksPath` to `.githooks`; `git check-ignore .gradle/lint-tools.properties` confirms it's ignored.
