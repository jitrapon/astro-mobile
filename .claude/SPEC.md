...
- [ ] 13a. Author `.githooks/pre-push` (bash, ported from `astro-calendar-service`) — run the security scanners over the commits this push adds (reachable from the pushed tip but not already on the remote): the item 12 semgrep ruleset (SAST) and the item 13 secret scanner, each warn-and-skip if its CLI isn't installed so the other still runs. Belt-and-suspenders for the CI security workflow (item 15); skippable with `git push --no-verify`. (Depends on items 12–13 establishing the semgrep + secret-scan configs, and on item 4d's `installGitHooks`.)
