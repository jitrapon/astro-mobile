...
- [ ] A5. `./gradlew tasks --all | grep ReleaseAndroidTest` lists the managed-device task, and `./gradlew :androidApp:check` does **not** reach it. With the four credentials unset, invoking that task fails with A4's named message rather than an install error — the end-to-end pairing A4 could only verify against `connectedReleaseAndroidTest`, since the managed device did not exist yet.
