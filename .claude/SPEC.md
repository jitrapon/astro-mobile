...
- [ ] A1. `./gradlew :androidApp:assembleRelease` succeeds and `androidApp/build/outputs/mapping/release/configuration.txt` no longer contains the AAPT-generated `-keep class androidx.compose.ui.tooling.PreviewActivity` rule. `./gradlew :androidApp:assembleDebug` still compiles `AppShellPreviews.kt`.
- [ ] A2. `configuration.txt` regenerated after the change names `proguard-rules.pro` as an originating file, and `./gradlew :androidApp:assemble` stays green on a machine with no keystore.
- [ ] A3. The audit is recorded in `proguard-rules.pro`'s header with the list of artifacts contributing rules and the verdict for each broad-rule category; `grep` over the regenerated `configuration.txt` for `-dontoptimize` / `-dontshrink` / `-dontobfuscate` returns what the header claims.
- [ ] A4. Invoking the managed-device release test task with the four credentials unset fails with the named message rather than an install error; with them set it proceeds.
- [ ] A5. `./gradlew tasks --all | grep ReleaseAndroidTest` lists the managed-device task, and `./gradlew :androidApp:check` does **not** reach it.
...
- [ ] C3. An instrumented test asserts the top bar shows the delivered `title` and one switcher option per delivered option, with the active one selected.
- [ ] C4. An instrumented test asserts a `ShowScreen` effect shows the matching destination's screen, and that each other effect reaches its handler rather than being dropped.
...
- [ ] C7. Same build and Swift gates as C6, plus a runtime `ios-device-debug` pass exercising a switcher selection, a tab whose action opens a URL, and an event tap — the paths no compile-time gate reaches.
