...
- [ ] A5b. Fail an instrumented run that executes zero tests: a verification task, finalizing every `AndroidTestTask`, that reads the run's JUnit XML and fails when no result file exists or the summed `tests` count is zero — naming the likely cause (a runner crash before the first test, which UTP reports as a pass).
...
- [ ] A5b. Reproduce the vacuous pass and watch the gate catch it: temporarily disable the generated rules (drop them from `proguardFiles`) so the runner crashes before its first test, and confirm the managed-device invocation now **fails** naming the zero-test result; restore and confirm it passes.
