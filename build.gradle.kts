...
//
// Two separate, non-transitive configurations because the fat jars bundle their own runtime deps and
// pin conflicting kotlin-compiler-embeddable versions; resolving them together would force a clash.
...
dependencies {
    // `with-dependencies` (ktfmt) and `all` (detekt-cli) are the self-contained fat-jar classifiers.
...
tasks.register("installGitHooks") {
    ...
    doLast {
        ...
        // The hook scripts are authored in later scaffolding steps; chmod each only once it exists so
        // installing the hooks early (before the scripts land) still wires core.hooksPath cleanly.
